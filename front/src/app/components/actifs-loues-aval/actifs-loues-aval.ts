...
export class ActifsLouesAvalComponent implements OnInit {
   ...
   *
   * <p>Un actif loue en aval l'est sur la duree du bail, non sur l'exercice consulte : le contrat court d'une annee sur l'autre.
  ...
  telechargerGabarit(): void {
    const exemples = [
      ...
      ...colonnesIdentite('MS3C13LA', 'ACT-0044'),
  ...
  private remigrerParReferentiel(): void {
    const MARQUEUR = marqueurEcran('actifs_loues_aval');
    ...
    const { lignes, corrigees } = remigrerLignes(
      ...
      adaptateurStandard<EmissionActifAval>({
      reference: 'reference',
