...
export class TransformationProduitsComponent implements OnInit {
  ...
  private remigrerParReferentiel(): void {
    const MARQUEUR = marqueurEcran('transformation_produits');
  ...
  private readonly CATEGORIE_REPLI = 'transformation-produits';
   ...
   */
  get lignesServeur(): EmissionTransformation[] {
    return mesuresDeLEcran(
      this.mesuresServeur, { numeroGhg: 10 }, this.exerciceActif, this.perimetreAffiche
    ).map(m => ligneDeLaBase(m, posteParId('transformation-produits')?.libelle ?? this.CATEGORIE_REPLI) as unknown as EmissionTransformation);
  }
