...
/**
 * Phrase rendue sous le tableau, quand le périmètre a écarté des lignes.
 *
 * <p>Un tableau qui rétrécit sans explication se lit comme une perte. Ce compte
 * rendu dit ce qui a été mis de côté et pourquoi, de sorte qu'un écran vide
 * s'explique de lui-même.</p>
 *
 * @returns la chaîne vide quand rien n'a été écarté ni laissé sans rattachement.
 */
export function messagePerimetre(tri: TriPerimetre<unknown>,
                                 societe: string,
                                 exercice: number | null): string {
  const parts: string[] = [];

  if (tri.autreSociete) {
    parts.push(`${tri.autreSociete} ligne(s) relèvent d'une autre société`);
  }
  if (tri.autreExercice) {
    parts.push(`${tri.autreExercice} ligne(s) documentent un autre exercice`
      + (exercice === null ? '' : ` que ${exercice}`));
  }

  if (!parts.length && !tri.sansRattachement) return '';

  let message = parts.length
    ? `${parts.join(' et ')} : elles ne sont pas affichées ici.`
    : '';

  if (tri.sansRattachement) {
    message += `${message ? ' ' : ''}${tri.sansRattachement} ligne(s) ne portent `
      + `aucune société : elles restent affichées faute de pouvoir dire si elles `
      + `relèvent de ${societe}.`;
  }
